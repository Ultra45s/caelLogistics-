...
import { createBackup, generateTestData } from '../lib/db';
...
const AdminProfileView: React.FC<AdminProfileViewProps> = ({ profile, appData, onUpdate, onRestore }) => {
  ...
  const handleExportBackup = async () => {
    if (window.api?.exportBackup) {
      ...
      if (result.success) {
        alert('Cópia de segurança exportada com sucesso!');
  ...
  const handleImportBackup = async () => {
    if (window.api?.importBackup) {
      ...
      if (result.success) {
        alert('Base de dados importada com sucesso! O aplicativo será reiniciado.');
        ...
        <div className="glass-panel rounded-lg p-10 text-text-main relative overflow-hidden group border border-white/10 shadow-2xl">
          ...
          <div className="relative z-10">
            ...
            <div className="space-y-4">
              <button onClick={handleExportBackup} className="w-full bg-white/5 hover:bg-white/10 p-5 rounded-lg font-black uppercase tracking-widest text-[10px] flex items-center justify-center gap-3 transition-all border border-white/10 shadow-xl">
                <Download size={20} className="text-blue-400" /> Exportar Backup (Base de Dados)
              </button>
              <button onClick={handleImportBackup} className="w-full bg-white/5 hover:bg-white/10 p-5 rounded-lg font-black uppercase tracking-widest text-[10px] flex items-center justify-center gap-3 transition-all border border-white/10 shadow-xl">
                <Upload size={20} className="text-emerald-400" /> Importar Backup (Base de Dados)
              </button>
              <button onClick={() => generateTestData()} className="w-full bg-rose-600/10 hover:bg-rose-600/20 p-5 rounded-lg font-black uppercase tracking-widest text-[10px] flex items-center justify-center gap-3 transition-all border border-rose-500/20 text-rose-400 shadow-xl">
                <RotateCcw size={20} /> Resetar e Gerar Telemetria
              </button>
            </div>
          </div>
        </div>
